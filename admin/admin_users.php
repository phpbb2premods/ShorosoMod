...
$sql="SELECT * FROM " . USERS_TABLE . " WHERE username='".$HTTP_POST_VARS['username']."'";
		$resultat = $db->sql_query($sql) or message_die(GENERAL_ERROR, 'Erreur SQL.');
		while($level = $db->sql_fetchrow($resultat))
		{
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		if ($user_level == 1)
		{
			message_die(GENERAL_MESSAGE, $lang['no_edit_admin']);
		}
...
//-- fin mod : birthday --------------------------------------------------------
		...
		$group_priority = ( isset( $HTTP_POST_VARS['group_priority'] ) ) ? intval( $HTTP_POST_VARS['group_priority'] ) : '';
...
//-- fin mod : birthday --------------------------------------------------------
		...
		// Eff. Color Groups MOD
		$user_group_priority = intval( $this_userdata['group_priority'] );
...
//-- fin mod : birthday --------------------------------------------------------
	...
	else
	{
		...
		{
			if( $group_id != ' ' && strpos( $color_groups['g' . $group_id]['members'], ' ' . $user_id ) !== FALSE )
			{
				$matches++;
