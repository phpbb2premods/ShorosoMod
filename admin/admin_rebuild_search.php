 ...
 *
 *     $Id: admin_rebuild_search.php,v 2.2.2.0 2006/02/04 18:38:17 chatasos Exp $
 *
 ****************************************************************************/
...
define('REBUILD_SEARCH_VERSION', '2.2.2');
...
// check if we must disable the board
$disable_board = false;
$board_status_disabled = board_already_disabled();

if ( !$board_status_disabled )
{
...
if ( isset($HTTP_GET_VARS['cancel_button']) || isset($HTTP_POST_VARS['cancel_button'])  )
{
	...
	if ( !$db->sql_query($sql) )
	{
		message_die(GENERAL_ERROR, 'Could not update rebuild search data', '', __LINE__, __FILE__, $sql);
	}
...
if ( $mode == 'submit' || $mode == 'refresh' )
{
	...
	if ( $session_posts_processed < $session_posts_processing && $total_posts_processed < $total_posts )
	{
		...
		$form_parameters .= ( $disable_board ) ? '&disable_board=1' : '';
	...
	$template->assign_vars(array(
		...
		'BOARD_STATUS'		=> ( $disable_board || $board_status_disabled ) ? $lang['Board_disabled'] : $lang['Board_enabled'],
...
else	// show the input page
{
	...
	$template->assign_vars(array(
		...
		'L_DISABLE_BOARD_EXPLAIN'	=> $lang['Disable_board_explain'].'<br />'.(( $board_status_disabled ) ? $lang['Disable_board_explain_already'] : $lang['Disable_board_explain_enabled']),
		...
		'REFRESH_RATE'					=> $refresh_rate,
		'TIME_LIMIT'					=> $time_limit,
		'BOARD_DISABLED'				=> ( $board_status_disabled ) ? 'disabled="disabled" checked="checked"' : '',
