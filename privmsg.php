...
else if ( $mode == 'read' )
{
  ...
  // END PM Navigation MOD
	...
	else if ( $folder == 'sentbox' )
	{
		...
		// DEBUT MOD Rendre tous les MPs citables
		$quote_img = $post_icons['quote_img'];
		$quote = $post_icons['quote'];
		// FIN MOD Rendre tous les MPs citables
	...
	//
	$template->assign_vars(array(
		'MESSAGE_TO' => do_color_groups( $user_id_to, $username_to ),
		'MESSAGE_FROM' => do_color_groups( $user_id_from, $username_from ),
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	else 
	{
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
					...
					// COMMENTAIRE MOD Rendre tous les MPs citables : La requête qui suit a été modifiée.
					// -- DEBUT Enlevé
