 ...
 *
 *     $Id: functions_admin_rebuild_search.php,v 2.2.2.0 2006/02/04 18:38:17 chatasos Exp $
 *
 ****************************************************************************/
...
function get_db_sizes()
{
	...
	else if( preg_match("/^mssql/", SQL_LAYER) )
	{
		...
		if( $result = $db->sql_query($sql) )
		{
			$db_size = ( $row = $db->sql_fetchrow($result) ) ? intval($row['dbsize']) : '';
			$search_size = '';
		}
		else
		{
...
// convert integer value to x Bytes string
...
// after/before include and the post_id
...
function disable_board($state)
{
	global $db;

	$sql = "UPDATE " . CONFIG_TABLE . "
		SET config_value = " . ( ( $state ) ? 1 : 0 ) . "
...
// check if the board is already disabled
function board_already_disabled()
{
	global $db;
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Could not check status of board', '', __LINE__, __FILE__, $sql);
	}

	$row = $db->sql_fetchrow($result);

	return ( $row['config_value'] && $row['config_value'] == 1 ) ? true : false;
}
