...
if( $mode == 'editprofile' && !isset( $HTTP_POST_VARS['submit'] ) )
{
	$color_groups = get_color_groups();
	
	$group_ids = explode( ' ', $color_groups['group_ids'] );
	$matches = 0;

	foreach( $group_ids as $group_id )
	{
		if( $group_id != ' ' && strpos( $color_groups['g' . $group_id]['members'], ' ' . $userdata['user_id'] ) !== FALSE )
		{
			$matches++;
			$match_array['names'][] = $color_groups['g' . $group_id]['name'];
			$match_array['ids'][] = $group_id;
		}
	}

	if( $matches > 1 )
	{
		$template->assign_block_vars( 'switch_group_select', array() );
		$select_box = '<select name="group_priority">';

		foreach( $match_array['names'] as $key => $group_name )
		{
			$selected = ( $userdata['group_priority'] == $match_array['ids'][$key] ) ? '" selected="selected' : '';
			$select_box .= '<option value="' . $match_array['ids'][$key] . $selected .'">' . $group_name . '</option>';
		}
		$select_box .= '</select>';
		
		$template->assign_vars( array( 
			'GROUP_SELECT' => $select_box,
...
//-- modify
			...
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_icq, user_website, user_occ, user_from, user_interests, user_birthday, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey)
				VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . $user_birthday . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, ";
//-- fin mod : birthday --------------------------------------------------------
