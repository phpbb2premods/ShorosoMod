...
if( ( $total_categories = count($category_rows) ) )
{
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		$forum_moderators[$row['forum_id']][] = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '">' . do_color_groups( $row['user_id'], $row['username'] ) . '</a>';
	}
	$db->sql_freeresult($result);
	...
	if( $board_config['colorize_groups'] == '1' )
	{
		$colorgroups_legend = '';
		$group_array = explode( ' ', $color_groups['group_ids'] );

		foreach( $group_array as $group_id )
		{
			if( $group_id != '' )
			{
				$colorgroups_legend .= ' [ <span style="color: #' . $color_groups['g' . $group_id]['color'] . '">' . $color_groups['g' . $group_id]['name'] . '</span> ]';
			}
		}
	}
...
//-- fin mod : birthday --------------------------------------------------------
	...
	$template->assign_vars(array(
		...
		'NEWEST_USER' => sprintf($lang['Newest_user'], '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$newest_uid") . '">', do_color_groups( $newest_uid, $newest_user ), '</a>'), 
		'GROUP_LEGEND' => $colorgroups_legend,
				...
				for($j = 0; $j < $total_forums; $j++)
				{
					if ( $forum_data[$j]['cat_id'] == $cat_id )
					{
						...
						if ( $is_auth_ary[$forum_id]['auth_view'] )
						{
							...
							{
																$is_auth = auth(AUTH_READ, $forum_id, $userdata, $forum_topic_data);

								if( !$is_auth['auth_read'] )
								{
									$last_post = sprintf($lang['Forum_auth_index'], $is_auth['auth_read_type']);
								}
								else
								{
																	// Modified by Attached Forums MOD
								...
								$last_post .= '<br /> '; 
								$last_post .= ( $forum_data[$j]['user_id'] == ANONYMOUS ) ? ( ($forum_data[$j]['post_username'] != '' ) ? $forum_data[$j]['post_username'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $forum_data[$j]['user_id']) . '">' . do_color_groups( $forum_data[$j]['user_id'], $forum_data[$j]['username'] ) . '</a> ';
								// END Modified by Attached Forums MOD
								}
							}
							else
