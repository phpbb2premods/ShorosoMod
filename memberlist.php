...
$letter = ( isset($HTTP_GET_VARS['letter']) ) ? $HTTP_GET_VARS['letter'] : "";
...
switch( $mode )
{
	...
	case 'username':
		$order_by = "username $sort_order LIMIT $start, " . $board_config['topics_per_page'];
            if ($letter!="")
            {
            	$condition = " AND username > '$letter' ";
            }
            $template->assign_block_vars('switch_alpha', array('U_ALPHA_LINK' => append_sid("memberlist.$phpEx?mode=username")));
		break;
	case 'location':
	...
	if ($letter!="")
	{
		$condition = "username>'$letter' AND username<'$letter+1'"; 
	}
	$template->assign_block_vars('switch_alpha', array('U_ALPHA_LINK' => append_sid("memberlist.$phpEx?mode=username")));
		break;
}
...
$sql = "SELECT username, user_id, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_aim, user_yim, user_msnm, user_avatar, user_avatar_type, user_allowavatar 
	FROM " . USERS_TABLE . "
	WHERE user_id <> " . ANONYMOUS . $condition. "
	ORDER BY $order_by";
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$template->assign_block_vars('memberrow', array(
			...
			'ROW_COLOR' => '#' . $row_color,
			'ROW_CLASS' => $row_class,
			'USERNAME' => do_color_groups( $user_id, $username ),
			'FROM' => $from,
			'JOINED' => $joined,
			'POSTS' => $posts,
